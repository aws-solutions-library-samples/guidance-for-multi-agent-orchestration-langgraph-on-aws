...
from models import TroubleshootingRequest, TroubleshootingResponse, TroubleshootingSolution, TroubleshootingStep
...
class SolutionGeneration(BaseModel):
    ...
    solutions: List[TroubleshootingSolution] = Field(description="Troubleshooting solutions")
...
class TroubleshootingAgent:
    ...
    def _create_troubleshooting_tools(self):
        ...
        async def search_faq(query: str, limit: int = 3) -> str:
            ...
            try:
                ...
                if results:
                    faq_list = []
                    for item in results:
                        faq_list.append(f"- Q: {item.get('question', 'N/A')} A: {item.get('answer', 'N/A')}")
            ...
            try:
                ...
                if results:
                    ...
                    for guide in results:
                        guide_list.append(f"- {guide.get('title', 'Guide')}: {guide.get('content', '')[:100]}...")
        ...
        async def search_product_help(product_name: str, category: str = None) -> str:
            ...
            try:
                ...
                if results:
                    help_list = []
                    for item in results:
                        help_list.append(f"- {item.get('topic', 'Help')}: {item.get('content', '')[:100]}...")
            ...
            try:
                ...
                if results:
                    ...
                    for issue in results:
                        issue_list.append(f"- {issue.get('issue_type', 'Issue')}: {issue.get('solution', '')[:100]}...")
        ...
        @tool
        async def get_warranty_info(product_name: str = None, category: str = None) -> str:
            """
            ...
            try:
                ...
                if results:
                    ...
                    for warranty in results:
                        warranty_list.append(f"- {warranty.get('product', 'Product')}: {warranty.get('warranty_period', 'N/A')} warranty")
            ...
            try:
                ...
                if results:
                    ...
                    for item in results[:5]:  # Limit to 5 results
                        help_list.append(f"- {item.get('title', 'Result')}: {item.get('content', '')[:100]}...")
    ...
    def _create_state_graph(self):
        ...
        def generate_structured_response(state):
            ...
            try:
                structured_response = self.solution_llm.invoke([HumanMessage(content=structured_prompt)])
    ...
    async def process_request(self, request: TroubleshootingRequest) -> TroubleshootingResponse:
        ...
        try:
            ...
            if structured_output and hasattr(structured_output, 'solutions'):
                ...
                response_text = final_message.content if final_message else "No solutions available"
